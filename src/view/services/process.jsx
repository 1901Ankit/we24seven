    ...
    <section className="relative bg-gray-50 overflow-hidden py-6 px-4">
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10">
        {processes.map((process, index) => (
          <div
            key={index}
            className="relative group cursor-pointer transform transition-transform duration-500 hover:scale-105"
            onMouseEnter={() => setActiveCard(index)}
            onMouseLeave={() => setActiveCard(null)}
          >
            <div
              className="relative p-6  backdrop-blur-2xl border border-white/10 rounded-2xl
       ...
       "
              ...
              <p className="font-medium text-sm mb-6 leading-relaxed text-gray-600 ">
                {process.description}
              </p>
